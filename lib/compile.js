...
var transform = function () {
  ...
  function program(node, options, resume) {
    ...
    visit(node.elts[0], options, function (err, val) {
      ...
      getData("xVBIdPLquo", function (items) {
        var map = {};
    ...
    function setIDs(node, saveIDs, resume) {
      ...
      if (node !== null && (typeof node === "undefined" ? "undefined" : _typeof(node)) === "object") {
        ...
        mapList(keys, function (k, resume) {
          ...
          // this function.
          if (node[k].id) {
            //            console.log("[1] setIDs() ids=" + node[k].id);
            //            node[k].ids = [node[k].id];
            resume([node[k].id]);
          } else {
            // If not leaf, drill down.
            setIDs(node[k], saveIDs, function (ids) {
    ...
    try {
      ...
      transform(code, options, function (err, val) {
        if (err.length) {
          ...
          render(_data, function (val) {
            //            console.log("render() saveIDs=" + JSON.stringify(saveIDs, null, 2));
            setIDs(val, saveIDs, function (ids) {
              tex2SVG("\\ldots", function (e, svg) {
                var root = {};
